...
function getSystemPrompt(profileKey, pages) {
  ...
  return [
    ...
    '   PART 4 — pillar1, pillar2, pillar3 (bullet points, each 50–70 words): Each pillar is a named bullet. Format exactly: "Label Title: [evidence paragraph]". Label is 2–5 words in title case. Evidence must follow this three-step chain: (a) NAME the credential, tool, or achievement from the resume — but NEVER stop there; (b) EXPLAIN the underlying methodology, framework, or discipline the credential represents and what it enables in practice (e.g. CBAP → BABOK elicitation framework → distinguishes business needs from technical wants; PMP → structured risk and scope management; SQL → data validation logic not just querying); (c) MAP that methodology directly to a specific problem or requirement named in the JD — show what it concretely delivers for THIS employer. A pillar that only names a credential without explaining the methodology behind it is incomplete. Each pillar covers a DIFFERENT dimension of fit.',
...
async function build(){
  ...
  try {
    ...
    var prompt = [
      ...
      (function(){
        var lInstr = {
          short:  'COVER LETTER LENGTH — STRICT HARD LIMIT: 280-320 words. You MUST count every word before responding. If your draft exceeds 320 words, cut it down before returning JSON. Sections: openingParagraph (3 sentences, max 60 words), whyTeamParagraph (2-3 sentences, max 55 words), idealCandidateIntro (2 sentences, max 35 words), pillar1 and pillar2 only (40-50 words each — tight, evidence-packed) — set pillar3 to empty string "", closingParagraph (2 sentences, max 40 words). Total must be 280-320 words. No exceptions.',
          medium: 'COVER LETTER LENGTH — STRICT HARD LIMIT: 380-430 words. You MUST count every word before responding. If your draft exceeds 430 words, cut it down before returning JSON. Sections: openingParagraph (3-4 sentences, max 85 words), whyTeamParagraph (3-4 sentences, max 85 words), idealCandidateIntro (2 sentences, max 40 words), pillar1 + pillar2 + pillar3 (50-70 words each — be concise, every word must earn its place), closingParagraph (2 sentences, max 45 words). Total must be 380-430 words. No exceptions.',
          long:   'COVER LETTER LENGTH — STRICT HARD LIMIT: 500-560 words. You MUST count every word before responding. If your draft exceeds 560 words, cut it down before returning JSON. Sections: openingParagraph (4 sentences, max 100 words), whyTeamParagraph (4 sentences, max 100 words), idealCandidateIntro (2 sentences, max 45 words), pillar1 + pillar2 + pillar3 (70-90 words each), closingParagraph (2-3 sentences, max 60 words). Total must be 500-560 words. No exceptions.'
        }[clLength] || 'COVER LETTER LENGTH — STRICT HARD LIMIT: 380-430 words. You MUST count every word before responding. If your draft exceeds 430 words, cut it down. Sections: openingParagraph (max 85 words), whyTeamParagraph (max 85 words), idealCandidateIntro (max 40 words), each pillar (50-70 words), closingParagraph (max 45 words). No exceptions.';
        return lInstr;
      })(),
      '',
...
function renderAtsScore(ats){
  ...
  if(missing.length){
    h+='<hr class="improve-sep">'
      ...
      +'<div class="improve-custom-wrap"><label class="improve-custom-label">&#x1F4AC; Additional instructions for AI <span style="font-weight:400;opacity:.7">(optional)</span></label><textarea id="improveCustomNote" class="improve-custom-input" placeholder="e.g. Emphasise Snowflake experience, add more stakeholder management examples..." rows="3"></textarea></div>'
...
// \u2500\u2500 Tracker file storage helpers \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
// Files are stored in their own localStorage keys (tc_file_<id>_resume / _cover)
// so they don't bloat the main tracker array and bypass the 5MB limit per key.
...
function saveTrackerFile(entryId, type, dataURL, name, mimeType, size) {
  // type = 'resume' | 'cover'
  var key = 'tc_file_' + entryId + '_' + type;
  try {
    localStorage.setItem(key, JSON.stringify({ name: name, type: mimeType, size: size, data: dataURL }));
    return true;
  } catch(e) { return false; }
}
function loadTrackerFile(entryId, type) {
  var key = 'tc_file_' + entryId + '_' + type;
  try { return JSON.parse(localStorage.getItem(key)); } catch(e) { return null; }
}
function deleteTrackerFiles(entryId) {
  localStorage.removeItem('tc_file_' + entryId + '_resume');
  localStorage.removeItem('tc_file_' + entryId + '_cover');
}
function trackerDownloadFile(entryId, type) {
  var fileObj = loadTrackerFile(entryId, type);
...
async function addTrackerEntry(company, role, template, profile, atsScore, jd, resumeBlob, resumeName, clBlob, clName) {
  ...
  // Save files in their own keys (separate from main tracker array)
  var savedResumeName = null;
  var savedCoverName  = null;
  try {
    if (resumeBlob) {
      var resumeData = await blobToDataURL(resumeBlob);
      if (resumeData) {
        var ok = saveTrackerFile(entryId, 'resume', resumeData,
          resumeName || 'Resume.docx',
          resumeBlob.type || 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
          resumeBlob.size);
        if (ok) savedResumeName = resumeName || 'Resume.docx';
      }
    }
    if (clBlob) {
      var clData = await blobToDataURL(clBlob);
      if (clData) {
        var ok2 = saveTrackerFile(entryId, 'cover', clData,
          clName || 'Cover_Letter.docx',
          clBlob.type || 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
          clBlob.size);
        if (ok2) savedCoverName = clName || 'Cover_Letter.docx';
      }
    }
  } catch(e) { /* files not critical \u2014 entry still saves */ }
  ...
  entries.unshift({
    ...
    status:           'Applied',
    jd:               jd || '',
    resumeFileName:      savedResumeName,   // just the name; data lives in tc_file_<id>_resume
    coverLetterFileName: savedCoverName,    // just the name; data lives in tc_file_<id>_cover
  });
  saveTracker(entries);
  renderTracker();
}
...
function renderTracker() {
  ...
  entries.forEach(function(e) {
    ...
    if (e.resumeFileName)      h += '<button class="tj-jd-btn" data-dl-resume="'+esc(e.id)+'" title="Download Resume: '+esc(e.resumeFileName)+'" style="background:#dbeafe;border-color:#bfdbfe;color:#1d4ed8">📄 Resume</button>';
    if (e.coverLetterFileName) h += '<button class="tj-jd-btn" data-dl-cover="'+esc(e.id)+'" title="Download Cover Letter: '+esc(e.coverLetterFileName)+'" style="background:#d1fae5;border-color:#a7f3d0;color:#065f46">✉️ Cover</button>';
    h += '</div></div>';
  });
  list.innerHTML = h;
  ...
  list.querySelectorAll('[data-del]').forEach(function(btn) {
    btn.addEventListener('click', function(ev) {
      ...
      deleteTrackerFiles(id); // clean up file keys
  ...
  list.querySelectorAll('[data-dl-resume]').forEach(function(btn) {
    btn.addEventListener('click', function(ev) {
      ev.stopPropagation();
      trackerDownloadFile(btn.getAttribute('data-dl-resume'), 'resume');
    });
  });
  list.querySelectorAll('[data-dl-cover]').forEach(function(btn) {
    btn.addEventListener('click', function(ev) {
      ev.stopPropagation();
      trackerDownloadFile(btn.getAttribute('data-dl-cover'), 'cover');
    });
  });
}
